...
//        }
    ...
    public ResponseResult adminArticleDetail(Long id) {
        ...
        Article article = getById(id);
        //根据id查询文章tags
        List<Long> tagsList = articleTagService.list()
                .stream()
                .filter(articleTag -> articleTag.getArticleId().equals(id))
    ...
    public ResponseResult updateArticle(UpdateArticleDto updateArticleDto) {
        ...
        //更新文章标签
        //删除原有的标签
        articleTagService.getBaseMapper().delete(new LambdaQueryWrapper<ArticleTag>()
